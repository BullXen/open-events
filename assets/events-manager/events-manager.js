...
// Se le "Date antecedenti" sono disattivate dall'admin (default), nessun
// campo data del form può selezionare o navigare a un giorno/mese/anno
// precedente a oggi.
const emAllowPastDates = (typeof openEventsManager === 'undefined') || !!openEventsManager.allowPastDates;

function emTodayMidnight() {
    const d = new Date();
    d.setHours(0, 0, 0, 0);
    return d;
}
...
class EmDatePicker {
    constructor($trigger, $input, minDate = null) {
        this.$trigger = $trigger;
        this.$input = $input;
        this.uid = ++emDatePickerUid;
        this.minDate = emAllowPastDates ? minDate : emTodayMidnight();
    ...
    // Impone un limite inferiore selezionabile (es. la Data Fine non può
    // precedere la Data Inizio scelta). Se la data già selezionata cade
    // prima del nuovo limite, la aggiorna al limite stesso.
    setMinDate(d) {
        this.minDate = d;
        if (this.minDate && this.selected && this.selected.getTime() < this.minDate.getTime()) {
            this.selected = new Date(this.minDate);
    ...
    _buildHtml() {
        ...
        const minYearFloor = this.minDate ? this.minDate.getFullYear() : null;
        const minMonthFloor = this.minDate ? this.minDate.getMonth() : null;
        const monthOpts = months.map((m, i) => {
            const disabled = minYearFloor !== null && this.viewYear === minYearFloor && i < minMonthFloor;
            return `<option value="${i}"${disabled ? ' disabled' : ''}>${m}</option>`;
        }).join('');

        // Solo i prossimi 5 anni da oggi; si estende solo se serve a includere
        // un anno già selezionato/in vista fuori da questo range (es. un
        // evento passato aperto in modifica), altrimenti resta fisso.
        const todayYear = new Date().getFullYear();
        let minYear = Math.min(todayYear, this.viewYear);
        let maxYear = Math.max(todayYear + 4, this.viewYear);
        if (minYearFloor !== null) minYear = Math.max(minYear, minYearFloor);
        let yearOpts = '';
...
<div class="em-dp">
    <div class="em-dp-header">
        <div class="em-dp-monthyear">
            <select class="em-dp-month-select" aria-label="Mese">${monthOpts}</select>
            <select class="em-dp-year-select" aria-label="Anno">${yearOpts}</select>
        </div>
        <div class="em-dp-nav">
            <button type="button" class="em-dp-prev" aria-label="Mese precedente">&#9664;</button>
            <button type="button" class="em-dp-next" aria-label="Mese successivo">&#9654;</button>
        </div>
    </div>
    ...
    _renderDays() {
        ...
        const $yearSelect = this.$dp.find('.em-dp-year-select');
        if (!$yearSelect.find(`option[value="${this.viewYear}"]`).length) {
            // L'utente ha navigato (prev/next) oltre il range iniziale delle
            // opzioni: estende la select invece di lasciarla senza corrispondenza.
            const years = $yearSelect.find('option').map((_, o) => parseInt(o.value, 10)).get();
            let minYear = Math.min(...years, this.viewYear);
            if (minYearFloor !== null) minYear = Math.max(minYear, minYearFloor);
            const maxYear = Math.max(...years, this.viewYear);
            let yearOpts = '';
            for (let y = minYear; y <= maxYear; y++) yearOpts += `<option value="${y}">${y}</option>`;
            $yearSelect.html(yearOpts);
        }
        this.$dp.find('.em-dp-month-select option').each((i, o) => {
            o.disabled = minYearFloor !== null && this.viewYear === minYearFloor && i < minMonthFloor;
        });
        this.$dp.find('.em-dp-month-select').val(this.viewMonth);
        $yearSelect.val(this.viewYear);

        const atFloorMonth = minYearFloor !== null && this.viewYear === minYearFloor && this.viewMonth === minMonthFloor;
        this.$dp.find('.em-dp-prev').prop('disabled', atFloorMonth).toggleClass('is-disabled', atFloorMonth);
        ...
        const minTs = this.minDate ? this.minDate.getTime() : null;
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if (minTs !== null && ts < minTs) cl.push('is-disabled');
    ...
    _bindEvents() {
        ...
        this.$dp.on('click', '.em-dp-prev', (e) => {
            e.stopPropagation();
            const atFloor = this.minDate && this.viewYear === this.minDate.getFullYear() && this.viewMonth === this.minDate.getMonth();
            if (atFloor) return;
        ...
        this.$dp.on('click', '.em-dp-month-select, .em-dp-year-select', (e) => e.stopPropagation());

        this.$dp.on('change', '.em-dp-month-select', (e) => {
            this.viewMonth = parseInt(e.currentTarget.value, 10);
            this._renderDays();
        });

        this.$dp.on('change', '.em-dp-year-select', (e) => {
            this.viewYear = parseInt(e.currentTarget.value, 10);
            this._renderDays();
        });

        this.$dp.on('click', '.em-dp-day:not(.is-empty):not(.is-disabled)', (e) => {
            e.stopPropagation();
...
class EventsManagerHandler extends elementorModules.frontend.handlers.Base {
    ...
    initDatePickers() {
        ...
        this.$element.find('.em-date-trigger').each((_, el) => {
            ...
            const forId = $trigger.data('for');
            const $input = this.$element.find('#' + forId);
            if ($input.length) {
                this.datePickers[forId] = new EmDatePicker($trigger, $input);
            }
        });
